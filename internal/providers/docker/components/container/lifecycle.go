...
func (c *Container) Initialize(ctx context.Context, input *core.InitializeInput) (output *core.InitializeOutput, err error) {
	...
	// The generated container name intentionally matches the container
	// name generated by Docker Compose when the scale is set at 1. When we
	// address scaling containers, this will need to be updated to use a
	// different suffix for each container.
	name := c.DockerObjectName(spec.ContainerName.Value, "1")

	if err := c.removeExistingContainerByName(ctx, name); err != nil {
		return nil, fmt.Errorf("removing existing container %q: %w", name, err)
	}
